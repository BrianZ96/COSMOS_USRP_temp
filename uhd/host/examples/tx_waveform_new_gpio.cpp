...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    size_t spb;
    double rate, freq, gain, wave_freq, bw, lo_offset, stepSize,gpio_switching_samples;
    float ampl;
//    ifstream reader("MIMO-Drone-signal-120MS-120MHz-294KHzx128.txt");
//    ifstream reader("Drone-signal-200MS-120MHz-3us+17usCP-3us+09usCPx127.txt");
//    ifstream reader("Drone_signal_200MS_150MHz_4us_05usCP.txt");
//    ifstream reader("Drone_signal_50MS_46MHz_250.txt");
//    ifstream reader("MIMO_Drone_signal_100MS_100MHz_4us.txt");
//    ifstream reader("MIMO_Drone_signal_100MS_100MHz_SIMO_signal500us.txt");
    ...
    desc.add_options()
        ...
        ("gpio_switching_samples", po::value<double>(&gpio_switching_samples), "gpio switching for every 'gpio_switching_samples'  samples")
    ...
    uhd::time_spec_t start_time = uhd::time_spec_t(1.5);
    ...
    int gpio789=1;
    uint64_t timestamp_tx = md.time_spec.to_ticks(rate);
    ...
    while (true) {
        ...
        timestamp_tx =  timestamp_tx + spb;
//        std::cout << boost::format(" time_spec_now  %ld , time_spec_switch_cmd %ld") % md.time_spec.to_ticks(rate) % timestamp_tx << std::endl;
        md.time_spec = uhd::time_spec_t::from_ticks(timestamp_tx, rate);
        usrp->set_command_time(md.time_spec);
//	std::cout << 
	usrp->set_gpio_attr("FP0", "OUT", gpio789<<7, 0x380);
	
	std::cout<<before<<gpio789;
        //usrp->clear_command_time();
       	gpio789 = (gpio789+1)&7;//basic 8 bit counter
       // gpio789 = 1;
	std::cout<<after<<gpio789;

     //   std::cout << boost::format(" buuff[15]  %d ") % buff[15] << std::endl;
      ...
      //      buff[n] = wave_table(index += step);
        ...
        usrp->clear_command_time(); //Added here as suggested by Neel
